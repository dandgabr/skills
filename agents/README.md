...
| 35 | **qa-testing-specialist** | [`AGENT.md`](qa-testing-specialist/AGENT.md) | [`agent.yaml`](qa-testing-specialist/agent.yaml) | [`agent.json`](qa-testing-specialist/agent.json) | Agente Especialista em Garantia de Qualidade (QA), Automação de Testes Multi-Framework, Testes de Mutação (Mutation Testing), Fuzzing e Auditoria de Conformidade de Requisitos. |
...
| 37 | **code-researcher** | [`AGENT.md`](code-researcher/AGENT.md) | [`agent.yaml`](code-researcher/agent.yaml) | [`agent.json`](code-researcher/agent.json) | Subagente Especialista em Pesquisa em Base de Código, Varredura de Repositórios, Análise de Árvore Sintática Abstrata (AST), Símbolos, Dependências e Arquitetura de Software com ferramentas de leitura estrita. |
...
| 46 | **web-researcher** | [`AGENT.md`](web-researcher/AGENT.md) | [`agent.yaml`](web-researcher/agent.yaml) | [`agent.json`](web-researcher/agent.json) | Agente Especialista em Pesquisa na Web e Motores de Busca (Google, DuckDuckGo, Bing, SearXNG, Yahoo). Domina operadores booleanos avançados, Google Dorks, filtros de domínio, tipos de arquivos, OSINT defensivo, triangulação de dados e verificação factual de fontes. |
